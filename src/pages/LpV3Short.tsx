            ...
            {currentStep === 1 ? (
              <div className="mb-6">
                ...
                <p className="text-white/60 text-base leading-relaxed max-w-md mx-auto">
                  Answer 8 quick questions to see where you stand — then get instant access to the 28-min GenAI readiness training.
                </p>
              </div>
            ) : (
...
export default function LpV3Short() {
  ...
  const openQuiz = useCallback((source: string = "unknown") => {
    ...
    if (typeof window.fbq === "function") {
      window.fbq("track", "ViewContent", { content_name: "LpV3Short Quiz", source });
    }
  }, []);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    fetch(WEBHOOK_URL, { keepalive: true,
      ...
      body: JSON.stringify({
        ...
        quiz_call_interest: label(7), quiz_salary: label(8), quiz_source: "lp-v3-short",
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#f5f0eb" }}>
      ...
      <button
        type="button"
        onClick={() => {
          trackEvent("cta_click", { cta_label: "Get the GenAI Readiness Roadmap", cta_section: "sumo_bar", page_path: window.location.pathname });
          openQuiz("sumo_bar");
        }}
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-10 sm:py-16">
        <div className="max-w-3xl text-center">
          ...
          <button
            type="button"
            onClick={() => { trackEvent("cta_click", { cta_label: "Get the GenAI Readiness Roadmap", cta_section: "video_thumbnail", page_path: window.location.pathname }); openQuiz("video_thumbnail"); }}
